...
import { Checkbox, Modal, Text, Code, Loading } from "@geist-ui/react";
...
export const GetTracePage = (explorerUrl, getEtherscanApiUrl, getPepeyaUrl) => {
      ...
      <li>
        <details open>
          <summary>
            ...
            {prettyValueStr}
            <a href={`${explorerUrl}/address/${to}`}>{prettyAddress || to}</a>
            ::{prettyInput || input}
          </summary>
          <ul>
            {(calls || []).length > 0 &&
  ...
  const TracePage = () => {
    ...
    const [hasRetrievedTrace, setHasRetrieveState] = useState(false);
    const [hasUpdatedLocalStorage, setHasUpdatedLocalStorage] = useState(false);

    const getTraces = useCallback(async () => {
      const traceData = await fetch(getPepeyaUrl(txhash), {
        method: "GET",
      })
        .then((x) => x.json())
        .catch(() => null);
    ...
    return (
      <>
          ...
          <Modal.Content>
            {modalData && (
              <Text font="12px" type="secondary">
                <a href={`${explorerUrl}/address/${modalData.to || ""}`}>
                  {modalTitle}
                </a>
              </Text>
            )}
              ...
              modalData.data && (
                <>
                  <Text type="secondary" font="12px">
                    Event emitted from{" "}
                    <a href={`${explorerUrl}/address/${modalData.address}`}>
                      {modalData.address}
                    </a>
                  </Text>
                  <Text>Topics</Text>
            ...
            {modalData && modalData.type.includes("CALL") && modalData.input && (
              <>
                ...
                {modalData.resFragment &&
                  modalData.resFragment.result.length > 0 && (
                    <>
        ...
        <GenericPage>
          ...
          {executionTrace === null && !invalidTxHash && !hasRetrievedTrace && (
